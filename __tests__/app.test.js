...
describe(Endpoints.API_END, () => {
  test('returns status 200 with endpoints as json object', async () => {
    const json = await fs.readFile(FilePaths.ENDPOINTS_JSON, 'utf-8');
  ...
  describe('GET all', () => {
    ...
    test('articles returned in descending date order by default', async () => {
      ...
      expect(body.articles).toBeSortedBy(
        DBTableDefs.Articles.Fields.created_at,
        {
          descending: true,
        }
      );
    ...
    test('articles returned in descending author order when desc used', async () => {
      const { body } = await request(app)
        .get(
          `${Endpoints.ARTICLES_END}?${QueryTypes.sortBy}=${DBTableDefs.Articles.Fields.author}&${QueryTypes.order}=desc`
        )
        .expect(200);

      expect(body.articles).toBeSortedBy(DBTableDefs.Articles.Fields.author, {
        descending: true,
      });
    });
    test('articles returned in ascending author order when asc used', async () => {
      const { body } = await request(app)
        .get(
          `${Endpoints.ARTICLES_END}?${QueryTypes.sortBy}=${DBTableDefs.Articles.Fields.author}&${QueryTypes.order}=asc`
        )
        .expect(200);
    ...
    test('articles returned in descending title order when desc used', async () => {
      const { body } = await request(app)
        .get(
          `${Endpoints.ARTICLES_END}?${QueryTypes.sortBy}=${DBTableDefs.Articles.Fields.title}&${QueryTypes.order}=desc`
        )
        .expect(200);

      expect(body.articles).toBeSortedBy(DBTableDefs.Articles.Fields.title, {
        descending: true,
      });
    });
    test('articles returned in ascending title order when asc used', async () => {
      const { body } = await request(app)
        .get(
          `${Endpoints.ARTICLES_END}?${QueryTypes.sortBy}=${DBTableDefs.Articles.Fields.title}&${QueryTypes.order}=asc`
        )
        .expect(200);
    ...
    test('articles returned in descending votes order when desc used', async () => {
      const { body } = await request(app)
        .get(
          `${Endpoints.ARTICLES_END}?${QueryTypes.sortBy}=${DBTableDefs.Articles.Fields.votes}&${QueryTypes.order}=desc`
        )
        .expect(200);

      expect(body.articles).toBeSortedBy(DBTableDefs.Articles.Fields.votes, {
        descending: true,
      });
    });
    test('articles returned in ascending votes order when asc used', async () => {
      const { body } = await request(app)
        .get(
          `${Endpoints.ARTICLES_END}?${QueryTypes.sortBy}=${DBTableDefs.Articles.Fields.votes}&${QueryTypes.order}=asc`
        )
        .expect(200);
    ...
    test('articles returned in descending topics order when desc used', async () => {
      const { body } = await request(app)
        .get(
          `${Endpoints.ARTICLES_END}?${QueryTypes.sortBy}=${DBTableDefs.Articles.Fields.topic}&${QueryTypes.order}=desc`
        )
        .expect(200);

      expect(body.articles).toBeSortedBy(DBTableDefs.Articles.Fields.topic, {
        descending: true,
      });
    });
    test('articles returned in ascending topics order when asc used', async () => {
      const { body } = await request(app)
        .get(
          `${Endpoints.ARTICLES_END}?${QueryTypes.sortBy}=${DBTableDefs.Articles.Fields.topic}&${QueryTypes.order}=asc`
        )
        .expect(200);
    ...
    test('search by topic returns articles in descending votes order when sorted by desc', async () => {
      const query = 'mitch';
      const { body } = await request(app).get(
        `${Endpoints.ARTICLES_END}?${QueryTypes.topic}=${query}&${QueryTypes.sortBy}=${DBTableDefs.Articles.Fields.votes}&${QueryTypes.order}=desc`
      );
    ...
    test('search by topic returns articles in ascending votes order when sorted by asc', async () => {
      const query = 'mitch';
      const { body } = await request(app).get(
        `${Endpoints.ARTICLES_END}?${QueryTypes.topic}=${query}&${QueryTypes.sortBy}=${DBTableDefs.Articles.Fields.votes}&${QueryTypes.order}=asc`
      );
...
async function commentWithIdExists(commentId) {
  const result =
    await connection.query(`SELECT * FROM ${DBTableDefs.Comments.name}
  WHERE ${DBTableDefs.Comments.Fields.id} = ${commentId}`);
